...
(function () {
  ...
  // ----------------------------------------------------------
  // [1.0.1] РЕГИСТРАЦИЯ (Исправлено)
  // ----------------------------------------------------------
  function register() {
    // Напрямую обращаемся к глобальному объекту
    window.AdultPlugin = window.AdultPlugin || {};
    if (typeof window.AdultPlugin.registerParser === 'function') {
  ...
  // Пробуем зарегистрировать сразу или ждем появления API
  if (!register()) {
    var attempts = 0;
