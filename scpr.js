(function () {
  ...
  var VERSION = '1.1.0';
  var NAME    = 'scpr';
  var HOST    = 'https://video-scat.com';
  var TAG     = '[' + NAME + ']';

  var VIDEO_RULES = [
    { label: 'HD', re: /https?:\/\/scatporn\.club\/[^"'\s]+\.mp4/ },
    { label: 'MP4', re: /['"](https?:\/\/[^"'\s]+\.mp4[^"'\s]*)['"]/ },
    { label: 'HLS', re: /['"](https?:\/\/[^"'\s]+\.m3u8[^"'\s]*)['"]/ }
  ];

  var CARD_SELECTORS = ['div.th', '.video-item', '.item'];
  ...
  function parseCard(el) {
    var a=el.querySelector('a[href]'); if(!a) return null;
    var rawHref = a.getAttribute('href') || '';
    
    // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Извлекаем реальный URL видео из /pornvideos?url=/ru/videos/...
    var href = '';
    var urlMatch = rawHref.match(/[?&]url=([^&#]+)/);
    if (urlMatch) {
      href = cleanUrl(decodeURIComponent(urlMatch[1]));
    } else {
  ...
  function parsePlaylist(html) {
    ...
    if(!items||!items.length) items=doc.querySelectorAll('a[href*="/videos/"], a[href*="pornvideos"]');
    if(items) for(var i=0;i<items.length;i++){var c=parseCard(items[i]);if(c)res.push(c);}
    return res;
  }
  function buildUrl(type,value,page) {
    page=parseInt(page,10)||1; var url=HOST+'/ru/';
  ...
  var ScatParser = {
    ...
    search:function(p,s,e){
      ...
      httpGet(buildUrl('search',q,pg),function(html){var r=parsePlaylist(html);s({title:'VideoScat: '+q,results:r,collection:true,total_pages:r.length>=20?pg+1:pg});},e);
    },
  ...
  function tryRegister(){if(window.AdultPlugin&&window.AdultPlugin.registerParser){window.AdultPlugin.registerParser(NAME,ScatParser);return true;}return false;}
