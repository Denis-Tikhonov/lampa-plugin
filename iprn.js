...
// Worker   : inporn.com, zbporn.com, nvms*.cdn.privatehost.com,
//            cdn.privatehost.com, cdnth.zbporn.com
//            — все должны быть в ALLOWED_TARGETS
// Headers  : Referer: https://inporn.com/  (обязателен для CDN)
// Redirect : zbporn.com/get_file/ → 302 → privatehost.com CDN
//            Worker должен следовать редиректам (mode: follow)
// =============================================================
// Изменения:
//   [1.0.0] Начальная версия
//           Каталог: .item (KVS-стандарт) + fallback a[href*="/video/"]
//           Qualities: S1 kt_player var video_url / flashvars
//                      S2 JSON-блок "sources":[...]
//                      S3 regex mp4-brute
//           Пагинация: &page=N
// =============================================================
...
(function () {
  ...
  // ============================================================
  // §1. КАТЕГОРИИ
  // KVS-сайт, категории статичны — добавляйте по мере надобности
  // ============================================================
  var CATEGORIES = [
  ...
  // ============================================================
  // §4. extractQualities
  // inporn использует KVS-плеер kt_player.
  // Видео-файл: zbporn.com/get_file/... → 302 → CDN privatehost.com
  //
  // S1: regex video_url: '...' / flashvars.video_url
  //     (kt_player стандарт KVS)
  // S2: JSON-блок "sources":[{file:..., label:...}]
  // S3: mp4-brute — все https://...mp4 в HTML
  // ============================================================
  function extractQualities(html) {
    var q = {};

    // S1 — kt_player: video_url / video_url_text / flashvars
    try {
      // Варианты записи в KVS:
      //   video_url: 'https://zbporn.com/get_file/...'
      //   video_url = 'https://...'
      //   flashvars.video_url = '...'
      var patterns = [
        /video_url\s*[:=]\s*['"]([^'"]+)['"]/i,
      ...
      for (var pi = 0; pi < patterns.length; pi++) {
        ...
        if (pm && pm[1]) {
          ...
          if (u && u.indexOf('http') === 0) {
            // KVS как правило даёт одно качество "High Quality"
            if (!q['High Quality']) q['High Quality'] = u;
            // Пытаемся угадать разрешение из имени файла
    ...
    // Дополнительно: video_url_360p / video_url_720p и т.д.
    try {
      var resRe = /video_url_(\w+)\s*[:=]\s*['"]([^'"]+)['"]/gi;
      var rm;
      while ((rm = resRe.exec(html)) !== null) {
        var label = rm[1]; // "360p", "720p", "1080p", "240p" ...
        var url   = cleanUrl(rm[2]);
        if (url && url.indexOf('http') === 0 && !q[label]) {
          q[label] = url;
        }
      }
    } catch (e) {
      console.warn(TAG, 'S1b multi-quality error:', e.message || e);
    }
    ...
    if (!Object.keys(q).length) {
      try {
        ...
        for (var sk = 0; sk < srcKeys.length; sk++) {
          var sIdx = html.lastIndexOf(srcKeys[sk]);
          if (sIdx === -1) continue;
          var aStart = html.indexOf('[', sIdx);
  ...
  // ============================================================
  // §5. ПАРСИНГ КАРТОЧЕК
  // KVS: .item (стандартный контейнер KVS)
  //       + fallback a[href*="/video/"]
  // Постер: img[src] / img[data-src] / img[data-original]
  // ============================================================
  ...
  // ============================================================
  // §6. URL BUILDER
  // Main:   https://inporn.com/
  // Search: https://inporn.com/?q={query}&page=N
  // Cat:    https://inporn.com/categories/{slug}/&page=N
  // Pag:    &page=N
  // ============================================================
    ...
    qualities: function (videoPageUrl, success, error) {
      ...
      httpGet(videoPageUrl, function (html) {
        ...
        if (keys.length > 0) {
          ...
          error('Видео не найдено — проверьте Worker (zbporn.com + privatehost.com)');
        }
      }, error);
    },
  };
  ...
  function tryRegister() {
    if (window.AdultPlugin && typeof window.AdultPlugin.registerParser === 'function') {
      window.AdultPlugin.registerParser(NAME, IprnParser);
