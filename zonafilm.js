 ...
 * =============================================================================
 * Данный файл представляет собой плагин для медиацентра Lampa (Android TV / Smart TV),
 * предоставляющий доступ к каталогу adult-сайтов через универсальный парсер.
 * ============================================================================ */
"use strict";
// [ВСТАВЬТЕ СЮДА МОДУЛЬ ЛОГИРОВАНИЯ]
/* ==========================================================================
 * КЛАСС PluginLogger — простой модуль логирования для отладки плагина.
 * Хранит до 100 последних записей с таймстампом и данными.
 * ========================================================================== */
class PluginLogger {
// Конструктор: инициализирует пустой массив логов и лимит 100
  constructor() {
    this.logs = [];
    this.maxLogs = 100;
  }

// log(message, data) — добавляет запись с таймстампом в лог и в console.log
...
// getLogs() — возвращает массив записей лога
  getLogs() {
    return this.logs;
  }

// clearLogs() — очищает массив логов
  clearLogs() {
    this.logs = [];
  }
}
...
/* ==========================================================================
 * BABEL ПОЛИФИЛЛЫ И ХЕЛПЕРЫ (_toConsumableArray, _slicedToArray, _typeof и др.)
 * Сгенерировано Babel-транспайлером для совместимости со старыми движками.
 * ========================================================================== */
...
/* ==========================================================================
 * REGENERATOR RUNTIME — поддержка async/await и генераторов
 * Copyright (c) 2014-present, Facebook, Inc. (MIT license)
 * ========================================================================== */
...
/* ==========================================================================
 * ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ РАБОТЫ С ОБЪЕКТАМИ И КЛАССАМИ
 * (ownKeys, _objectSpread, _defineProperty, _classCallCheck, _createClass и др.)
 * ========================================================================== */
...
/* ==========================================================================
 * ОСНОВНАЯ ЛОГИКА UI — ИНТЕГРАЦИЯ С ФРЕЙМВОРКОМ LAMPA
 * Включает: выбор качества, превью, воспроизведение, меню, карточки
 * ========================================================================== */
!(function (e, t, a, n, r, i, o, s) {
  !(function () {
    ...
    var t,
      a,
      n = new Lampa.Reguest();
  // r(e) — Выбор видео-потока нужного качества
  // Из настроек Lampa берёт качество по умолчанию (1080p), ищет совпадение в объекте e.
  // Если не найдено — берёт первое доступное качество.
    function r(e) {
      var t,
        a = Lampa.Storage.get("video_quality_default", "1080") + "p" // качество по умолчанию;
      if (e) {
  ...
  // i() — Скрывает видео-превью при наведении на карточку.
  // Останавливает воспроизведение, скрывает элемент, сбрасывает таймаут.
    function i() {
      if ((clearTimeout(t), a)) {
        var e,
          n = a.find("video");
    ...
    var o,
      // sourceTitle(e) — извлекает название сайта из домена (до первой точки)
      s = {
        sourceTitle: function (e) {
          return Lampa.Utils.capitalizeFirstLetter(e.split(".")[0]);
      // play(e) — воспроизводит видео через плеер Lampa.
      // Если e.json — сначала запрашивает качества через API, иначе — напрямую.
        },
        play: function (e) {
      ...
      // fixCards(e) — подготавливает карточки: poster, img, очистка HTML-сущностей
        },
        fixCards: function (e) {
      ...
      // preview(e, n) — показывает GIF-превью при наведении на карточку (задержка 1.5с)
        },
      ...
      // fixList(e) — нормализует список: заменяет пустой quality на time
        hidePreview: i,
        fixList: function (e) {
          return (
            e.forEach(function (e) {
      ...
      // menu(t, a) — контекстное меню: «Похожие», «Модель»
        },
        menu: function (t, a) {
          var n = [];
...
/* ==========================================================================
 * API СЕРВИСНЫЙ СЛОЙ (объект l): menu, view, playlist, main, search, qualitys
 * ========================================================================== */
    var l = new (function () {
      var e = this,
        t = new Lampa.Reguest();
// l — API сервис: экземпляр с методами загрузки данных сайтов
      ((this.menu = function (e, t) {
        if (o) return e(o);
        var a = AdultJS.Menu();
        a ? e((o = a)) : t(a.msg);
      }),
      // view(e, t, a) — загружает каталог по URL, преобразует в формат Lampa
      ...
      // playlist(t, a, n) — параллельно загружает плейлисты всех сайтов.
      // Таймаут 8 секунд на каждый запрос. Использует Lampa.Status для отслеживания.
      ...
      // main(e, t, a) — главная страница: загружает все плейлисты (пустой запрос)
        (this.main = function (e, t, a) {
          this.playlist("", t, a);
        }),
      // search(e, t, a) — поиск по всем сайтам (кодирует запрос в URL)
        (this.search = function (e, t, a) {
          this.playlist("?search=" + encodeURIComponent(e.query), t, a);
        }),
      // TODO: плагин не имеет механизма повтора запросов при ошибках (no retry logic)
        (this.qualitys = function (e, t, a) {
      // qualitys(e, t, a) — получает качества видео для заданного URL через AdultJS.Invoke
          AdultJS.Invoke(e)
            .then(t)
            .catch(function (t) {
        ...
        (this.clear = function () {
      // clear() — очищает активные HTTP-запросы
          t.clear();
        }));
    })();
/* ==========================================================================
 * UI КОМПОНЕНТЫ: c(t) — InteractionMain, u(t) — InteractionCategory
 * ========================================================================== */
    function c(t) {
    // c(t) — Фабрика InteractionMain: создаёт компонент главной страницы
    // со списком плейлистов всех сайтов.
    ...
    function u(t) {
    // u(t) — Фабрика InteractionCategory: создаёт компонент категории
    // с фильтрацией, поиском, пагинацией и превью видео.
      var a,
...
/* ==========================================================================
 * ИНИЦИАЛИЗАЦИЯ ПЛАГИНА: меню, ParentalControl, компоненты, настройки
 * ========================================================================== */
...
/* ==========================================================================
 * HTTP КЛАСС (статические методы): ensureHeaders, Get, Native
 * На Android — нативный HTTP-клиент Lampa; в браузере — fetch()
 * ========================================================================== */
  var l =
      ((e = (function () {
        function e() {
        ...
        return _createClass(e, null, [
          {
            // ensureHeaders(e) — добавляет User-Agent Chrome 130 если не задан
            key: "ensureHeaders",
            value: function (e) {
          ...
          {
            // Get(url, headers, charset) — async GET-запрос.
            // На Android — нативный клиент, в браузере — fetch().
            key: "Get",
            value:
              ((t = _asyncToGenerator(
                      ...
                      switch (t.n) {
                        ...
                        case 1:
                          return (
                            ...
                            (t.n = 2),
                            fetch(a, o) // FIXME: будет заблокирован CORS при запуске в браузере!
                          );
                        case 2:
                          if (((s = t.v), null == r)) {
                            t.n = 4;
          ...
          {
            // Native(url, headers, charset) — нативный HTTP-запрос через Lampa.Reguest.
            // FIXME: зависит от window.Lampa.Reguest — работает только внутри Lampa.
            key: "Native",
 ...
 * ========================================================================== */
    l.GetWithMirrorFallback = (function () {
      ...
      return async function (url, mirrors, headers, charset) {
        ...
        var errors = [];
        for (var idx = 0; idx < orderedMirrors.length; idx++) {
          var mirror = orderedMirrors[idx];
          try {
            // Безопасная замена hostname в URL
            var mirrorUrl = _swapHost(url, mirror);
            var result = await l.Get(mirrorUrl, headers, charset);

            // Успех — запоминаем это зеркало
            _lastWorkingMirror = mirror;
            console.log("[Mirror] OK: " + mirror);
            return result;
          } catch (error) {
            errors.push({ mirror: mirror, error: error.message || String(error) });
            console.warn("[Mirror] Failed: " + mirror + " — " + (error.message || error));
            continue;
          }
        }
        ...
        throw new Error(
          "[Mirror] All mirrors failed for xnxx: " +
       ...
       * Работает корректно даже если старый хост содержит подстроки,
       * т.к. использует URL-парсинг, а не String.replace.
       */
      function _swapHost(url, newHost) {
        try {
          var parsed = new URL(url);
          parsed.protocol = newHost.startsWith("https") ? "https:" : "http:";
          parsed.host = newHost.replace(/^https?:\/\//, "");
          parsed.hostname = parsed.host; // синхронизируем
          return parsed.toString();
        } catch (e) {
          // Fallback: если URL не парсится — пробуем простую замену
          console.warn("[Mirror] URL parse failed, using fallback replace: " + url);
          return url;
        }
      }
    })();
 ...
 * ========================================================================== */
    c = (function () {
      return _createClass(
        ...
        [
          {
            // extract(html, regex, groupIndex) — извлекает строку из HTML по regex.
            // Возвращает matched-группу (по умолчанию 1) или null.
            key: "extract",
            value: function (e, t) {
              var a,
                n =
    ...
    // VideoItem — DTO для одного видео:
    // (name, video, picture, preview, time, quality, json, related, model)
    ...
    // PlaylistItem — DTO элемента меню:
    // (title, playlist_url, search_on, submenu)
      (_classCallCheck(this, e),
        (this.title = t),
        (this.playlist_url = a),
        n && (this.search_on = n),
...
/* ==========================================================================
 * ПАРСЕР BongaCams (класс d) — https://ukr.bongacams.com
 * ========================================================================== */
    d =
      ((t = (function () {
        function e() {
        ...
        return _createClass(e, [
          {
// Invoke(t) — загружает страницу BongaCams, возвращает меню + список моделей
            key: "Invoke",
            value:
              ((t = _asyncToGenerator(
          ...
          {
            key: "Playlist",
// Playlist(html) — парсит HTML, извлекает модели (имя, esid, превью, качество)
            value: function (e) {
              var t = [];
              return e && 0 !== e.length
                ? (e
          ...
          {
            key: "Menu",
// Menu(url) — строит меню: Новые, Пары, Девушки, Русские, Парни, Трансы
            value: function (t) {
              var a = e.host + "/",
                n = [
      ...
      t),
/* ==========================================================================
 * ПАРСЕР XV-RU (класс g) — https://www.xv-ru.com
 * ========================================================================== */
    ...
    g =
      ((a = (function () {
        ...
        return _createClass(e, [
          {
            key: "Invoke",
// Invoke(a) — диспетчер: просмотр видео (HLS + похожие) или каталог/поиск
            value:
              ((t = _asyncToGenerator(
          ...
          {
            key: "buildUrl",
// buildUrl(host, search, sort, cat, page) — строит URL каталога/поиска
          ...
          {
            key: "getLastMonth",
// getLastMonth() — возвращает прошлый месяц (YYYY-MM) для фильтра «лучшие»
            value: function () {
              var e = new Date();
              return (
          ...
          {
            key: "Playlist",
// Playlist(html) — парсит HTML, извлекает список видео
            value: function (t) {
              if (!t) return [];
              for (
          ...
          {
            key: "Menu",
// Menu(sort, cat) — строит меню сортировки и категорий
            value: function (t, a) {
              var n,
                r = e.host,
          ...
          {
            key: "StreamLinks",
// StreamLinks(html) — извлекает HLS-URL через html5player.setVideoHLS и похожие
            value: function (t) {
      ...
      a),
/* ==========================================================================
 * ПАРСЕР XNXX-RU (класс y) — https://www.xnxx-ru.com
 * ========================================================================== */
    y =
      ((n = (function () {
        function e() {
        ...
        return _createClass(e, [
          {
// Invoke(a) — диспетчер: видео или каталог
            key: "Invoke",
            value:
              ((t = _asyncToGenerator(
                _regenerator().m(function t(a) {
          ...
          {
            key: "buildUrl",
// buildUrl(host, search, page) — строит URL
          ...
          {
            key: "Playlist",
// Playlist(html) — парсит список видео
            value: function (t) {
              if (!t) return [];
              for (
...
// Menu() — строит меню (только поиск)
          ...
          {
            key: "StreamLinks",
// StreamLinks(html) — извлекает HLS и похожие видео
            value: function (t) {
              var a = c.extract(t, /html5player\.setVideoHLS\('([^']+)'\);/);
              if (!a) return new m({}, []);
              var n = [],
...
// Invoke(a) — диспетчер: видео или каталог
          ...
          {
            key: "buildUrl",
// buildUrl(host, search, sort, page) — строит URL (поддержка /most_popular?p=m)
          ...
          {
            key: "Playlist",
// Playlist(html) — парсит каталог видео
            value: function (html) {
              if (!html) return [];
          ...
          {
            key: "Menu",
// Menu(sort) — строит меню сортировки
            value: function (t) {
              var a = e.host + "/sbg";
              return [
          ...
          {
            key: "StreamLinks",
// StreamLinks(html) — regex для MP4 разных качеств (240p-4K)
            value: function (e) {
              for (
      ...
      r),
/* ==========================================================================
 * ПАРСЕР Chaturbate (класс b) — https://chaturbate.com
 * ========================================================================== */
    b =
      ((i = (function () {
        function e() {
        ...
        return _createClass(e, [
          {
// Invoke(a) — диспетчер: стрим (по username) или каталог моделей
            key: "Invoke",
            value:
              ((a = _asyncToGenerator(
                _regenerator().m(function t(a) {
          ...
          {
            key: "buildUrl",
// buildUrl(host, sort, page) — строит URL API roomlist (90 моделей/страница)
            value: function (e, t, a) {
              var n =
                e +
          ...
          {
            key: "Playlist",
// Playlist(json) — парсит roomlist, фильтрует public-шоу
            value: function (t) {
              if (!t) return [];
              for (
          ...
          {
            key: "Menu",
// Menu(sort) — меню фильтрации по полу (f/c/m/t)
            value: function (t) {
              var a,
                n = e.host + "/chu",
                r = [
          ...
          {
            key: "StreamLinks",
// StreamLinks(username) — async: загружает страницу модели, извлекает HLS m3u8
            value:
              ((t = _asyncToGenerator(
                _regenerator().m(function t(a) {
                  var n, r;
      ...
      i),
/* ==========================================================================
 * ПАРСЕР Eporner (класс f) — https://www.eporner.com
 * ========================================================================== */
    f =
      ((o = (function () {
        function e() {
...
// Invoke(a) — диспетчер: видео или каталог
          ...
          {
            key: "buildUrl",
// buildUrl(host, search, sort, cat, page) — строит URL
            value: function (e, t, a, n, r) {
              var i = "".concat(e, "/");
              return (
                t
...
// Playlist(html) — парсит каталог видео
          ...
          {
            key: "Menu",
// Menu(search, sort, cat) — меню с сортировкой и категориями
            value: function (t, a, n) {
              var r,
                i = e.host,
          ...
          {
            key: "StreamLinks",
// StreamLinks(host, url) — async: XHR-запрос видео, извлечение MP4-качеств
            value:
              ((t = _asyncToGenerator(
          ...
          {
            key: "convertHash",
// convertHash(hash) — конвертация 32-символьного hex в base36 (4×8 символов)
            value: function (e) {
              return (
          ...
          {
            key: "base36",
// base36(hex) — переводит hex-строку в основание 36
            value: function (e) {
      ...
      o);
/* ==========================================================================
 * УТИЛИТЫ NextHub: k() — шаблоны, w() — URL-пути, _() — HTML→DOM,
 * x() — XPath, C() — атрибуты
 * ========================================================================== */
  function k(e, t) {
// k(e, t) — заменяет {key} в строке e на значения из объекта t
  ...
  function w(e, t) {
// w(e, t) — объединяет базовый URL и путь, убирая дублирующиеся слеши
    var a = e.replace(/\/+$/, ""),
      n = t.replace(/^\/+/, "");
    return a + (n ? "/" + n : "");
  }
  function _(e) {
// _(e) — парсит HTML-строку в DOM-документ через DOMParser
    return new DOMParser().parseFromString(e, "text/html");
  }
  function x(e, t, a) {
// x(e, t, a) — выполняет XPath-запрос, возвращает первый элемент
    return e.evaluate(
      t,
      a || e,
      null,
  ...
  function C(e, t, a) {
// C(e, t, a) — получает атрибут элемента; поддерживает массив имён атрибутов
    if (!e) return "";
    if (Array.isArray(t)) {
      var n,
...
/* ==========================================================================
 * УНИВЕРСАЛЬНАЯ СИСТЕМА ПАРСИНГА NextHub (класс S)
 * Конфигурируемый парсер: buildListUrl, buildSearchUrl, buildModelUrl,
 * buildMenu, toPlaylist, extractStreams, Invoke
 * ========================================================================== */
  var S =
      ((s = (function () {
        return _createClass(
          ...
          [
            {
              key: "buildListUrl",
          // buildListUrl(cfg, page, sort, cat) — строит URL для списка видео.
          // Учитывает route, sort, cat, firstpage из конфигурации сайта.
              value: function (e, t, a, n) {
                var r,
                  i,
                  o,
            ...
            {
              key: "buildSearchUrl",
// buildSearchUrl(cfg, query, page) — строит URL поиска
              value: function (e, t, a) {
            ...
            {
              key: "buildModelUrl",
// buildModelUrl(cfg, model, page) — строит URL страницы модели
              value: function (e, t, a) {
                var n,
                  r =
                    null == e ||
            ...
            {
              key: "buildMenu",
          // buildMenu(cfg, sort, cat, hideSearch, href) — строит меню:
          // поиск, сортировки, категории. Для view-режима — также «Похожие».
              value: function (e, t, a) {
                var n,
                  r,
                  i,
            ...
            {
              key: "toPlaylist",
          // toPlaylist(dom, cfg) — извлекает список VideoItem из DOM
          // по XPath-правилам contentParse конфигурации.
              value: function (e, t) {
                var a,
                  n = t.contentParse,
                  r = (function (e, t, a) {
            ...
            {
              key: "extractStreams",
          // extractStreams(html, cfg) — извлекает потоковый URL из HTML.
          // Стратегии (по порядку): iframe → eval → nodeFile → regexMatch.
              value:
                ((t = _asyncToGenerator(
                  _regenerator().m(function e(t, a) {
                    var n,
            ...
            {
              key: "Invoke",
          // Invoke(url) — главный диспетчер NextHub:
          // view/related/model/search/list — определяет режим по параметру ?mode=
              value:
                ((e = _asyncToGenerator(
                  _regenerator().m(function e(t) {
                    var a,
      ...
      s),
/* ==========================================================================
 * КОНФИГУРАЦИИ NextHub САЙТОВ (массив P)
 * ========================================================================== */
    P = [
...
/* --- Xhamster: https://ru.xhamster.com --- */
      {
        enable: !0,
...
/* --- Lenporno: https://pepa.lenporno.xyz --- */
      {
        enable: !0,
...
/* --- 24video: https://sex.24videos.space --- */
      {
        enable: !0,
        displayname: "24video",
...
/* --- JopaOnline: https://jopaonline.mobi --- */
      {
        enable: !0,
...
/* --- NoodleMagazine: https://adult.noodlemagazine.com --- */
      {
        enable: !0,
...
/* --- Porno666: https://wwwp.porno666.news --- */
      {
        enable: !0,
...
/* --- PornoBriz: https://pornobriz.com --- */
      ...
      {
/* --- SemBatsa: https://sem.batsa.pro (ОТКЛЮЧЁН) --- */
        enable: !1,
...
/* --- Sosushka: https://gi.sosushka.vip --- */
      {
        enable: !0,
...
/* --- Porno365/PornoLab config --- */
      ...
      {
/* --- PornoAkt: https://a.pornoakt.club --- */
        enable: !0,
...
/* --- Rusvideos: https://sex.rusvideos.art --- */
      {
        enable: !0,
...
/* --- GayPornTube: https://www.gayporntube.com --- */
      {
        enable: !0,
...
/* --- Vtrahe: https://site.vtrahehd.tv (windows-1251) --- */
      {
        enable: !0,
        displayname: "Vtrahe",
...
/* ==========================================================================
 * КОНФИГУРАЦИЯ TRAHKINO — ЦЕЛЕВОЙ САЙТ (https://trahkino.me)
 * ========================================================================== */
      {
        enable: !0,
        ...
        view: {
          related: !0,
          regexMatch: {
            // TODO: паттерн regexMatch может сломаться при изменении URL-структуры trahkino.me
            // Ищет MP4-файлы разных качеств (1080p, 720p, 480p, 360p)
            matches: ["1080p", "720p", "480p", "360p"],
...
/* ==========================================================================
 * ЭКЗЕМПЛЯРЫ ПАРСЕРОВ И ГЛОБАЛЬНЫЙ ОБЪЕКТ AdultJS
 * ========================================================================== */
    z = new d(),
    L = new g(),
    j = new y(),
  ...
  !(function () {
    function e() {
    // e(t) — РОУТЕР: по URL определяет нужный парсер.
    // Порядок: BongaCams → XV-RU → XNXX-RU → SpankBang → Chaturbate →
    // Eporner → NextHub → поиск hostname в P → "unknown".
    ...
    window.AdultJS = {
      Menu: function () {
      // Menu() — формирует массив элементов меню:
      // 6 прямых парсеров + все включённые NextHub-сайты + кнопка лога.
        var e = [
      ...
      Invoke: function (t) {
      // Invoke(t) — точка входа: делегирует URL роутеру e(t)
        return e.apply(this, arguments);
      },
    };
  })();
