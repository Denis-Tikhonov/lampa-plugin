/**
 * ============================================================
 *  LAMPA PLUGIN — Trahkino v1.3.3
 * ============================================================
 *
 *  ИСПРАВЛЕНИЯ v1.3.3:
 *    ✅ Навигация пультом: создан отдельный контроллер 'zf_cards' 
 *       (вместо сломанного переопределения системного 'content')
 *    ✅ Постеры: переключены на api.codetabs.com (поддержка бинарных файлов)
    ...
    function CardsComp(object){
        ...
        this.onDataLoaded = function(items){
            ...
            items.forEach(function(m){
                // Картинки через codetabs (он корректно отдает бинарные данные в Lampa)
                var proxiedPoster = 'https://api.codetabs.com/v1/proxy?quest=' + encodeURIComponent(m.poster);
        ...
        this.start = function(){
            // Регистрируем СОБСТВЕННЫЙ контроллер с уникальным именем!
            Lampa.Controller.add('zf_cards', {
                toggle: function(){},
            ...
            Lampa.Controller.toggle('zf_cards');
        };
        ...
        this.stop = function(){
            // Обязательно чистить за собой
            Lampa.Controller.remove('zf_cards');
        };
