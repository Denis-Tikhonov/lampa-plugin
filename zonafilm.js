// ============================================================
// AdultJS Plugin for Lampa (Android TV)  v2.0.0
// ============================================================
"use strict";
...
var MirrorResolver = (function () {
  ...
  function testDomain(domain, timeoutMs) {
    ...
    return new Promise(function(resolve) {
      var startTs = Date.now(), controller = null, timer;
      ...
      fetch(domain, opts).then(function(res) {
        ...
        try {
          var er = new URL(domain).hostname.split('.').slice(-2).join('.');
          var fr = new URL(res.url).hostname.split('.').slice(-2).join('.');
          if (fr !== er) { resolve({ ok: false, reason: 'redirect_stub', domain: domain, latency: null }); return; }
        } catch(ex){}
  ...
  function testParser(siteKey, domain) {
    ...
    return fetch(domain.replace(/\/$/, '') + reg.testPath, { method: 'GET', headers: { 'User-Agent': 'Mozilla/5.0' } })
  ...
  function _resolveSite(domains) {
    ...
    function tryNext() {
      ...
      return testDomain(domain, 6000).then(function(r){ return r.ok ? domain : tryNext(); });
    }
    return tryNext();
  }
  ...
  function resolve(force) {
    ...
    if (!force && !_isCacheExpired(cache)) {
      _cache = cache;
      console.log('[MirrorResolver] Cache OK:', new Date(cache.updatedAt).toLocaleTimeString());
      return Promise.resolve(cache.mirrors);
    }
    console.log('[MirrorResolver] Resolving mirrors...');
    ...
    return Promise.all(promises).then(function(results) {
      ...
      if (failedKeys.length === 0) {
        ...
        console.log('[MirrorResolver] All OK:', mirrors);
        return mirrors;
      }
      console.warn('[MirrorResolver] Fails for:', failedKeys.join(', '));
      return _fetchRemoteMirrors().then(function(remote) {
        ...
        var offset = AdultJS_Config.cacheTtl.success - AdultJS_Config.cacheTtl.afterFail;
        var fail = { mirrors: mirrors, updatedAt: Date.now() - offset, hasFails: true };
...
var SourceHealth = (function () {
  ...
  function _load() {
    try { var r = localStorage.getItem(STORAGE_KEY); _state = r ? JSON.parse(r) : {}; }
    catch(e) { _state = {}; }
    return _state;
  }
  ...
  return {
    ...
    recordFail: function(name) {
      ...
      console.warn('[SourceHealth]', name, 'fail #' + s.failCount, 'blocked for', (BACKOFF_MS[idx]/60000).toFixed(0) + 'min');
    },
...
function requestWithRetry(url, options, maxRetries) {
  ...
  function attempt(n) {
    return fetch(url, options || {}).then(function(res) {
      ...
      console.log('[retry]', (n+1)+'/'+maxRetries, 'in', delay+'ms', err.message||err);
...
var AdultJS_ProxyAdapter = (function() {
  function buildStreamUrl(u) { return AdultJS_Config.workerUrl + '/stream?url=' + encodeURIComponent(u); }
  function isVideoUrl(u) { return /\.(mp4|m3u8|webm|ts)(\?|$)/i.test(u) || u.indexOf('/get_file/') !== -1 || u.indexOf('chunks.m3u8') !== -1 || u.indexOf('/hls/') !== -1; }
  function patchVideoUrl(u) { if (!AdultJS_Config.proxyEnabled || !u) return u; if (!isVideoUrl(u)) return u; return buildStreamUrl(u); }
...
// ── _regenerator (verbatim, no changes) ──────────────────────
...
!function (e, t, a, n, r, i, o, s) {
  !function () {
      ...
      menu: function(t, a) {
        ...
        a.related && n.push({ title: "Похожие", related: !0 }),
          ...
          Lampa.Select.show({
            ...
            onSelect: function(t) {
              t.model
                ? Lampa.Activity.push({ url: a.model.uri, title: "Модель - " + a.model.name, component: "sisi_view_" + e, page: 1 })
          ...
          .then(function(e) {
            e.list
              ? (e.results = s.fixList(e.list), e.collection = !0,
                 e.total_pages = e.total_pages || 30, s.fixCards(e.results), delete e.list, t(e))
              : a()
        ...
        var _run = function() {
          var activeSources = o.filter(function(src) {
            ...
            if (SourceHealth.isDisabled(name)) {
              console.log('[playlist] Skip', name, 'disabled until', new Date(SourceHealth.getDisabledUntil(name)).toLocaleTimeString());
              return false;
            }
            return true;
          });
          if (activeSources.length === 0) { console.warn('[playlist] All sources disabled'); n(); return; }
          ...
          activeSources.forEach(function(a) {
            ...
            var timer = setTimeout(function() {
              failed = true; SourceHealth.recordFail(sourceName); statusObj.error();
            }, 8e3);
            AdultJS.Invoke(a.playlist_url + r + i)
              .then(function(t) {
                ...
                if (t.list) {
                  ...
                  t.url = a.playlist_url; t.collection = true; t.line_type = "none";
                  t.card_events = { onMenu: s.menu, onEnter: function(e, t){ s.hidePreview(); s.play(t); } };
                  s.fixCards(t.results); delete t.list;
                  statusObj.append(a.playlist_url, t);
                } else { SourceHealth.recordFail(sourceName); statusObj.error(); }
              })
              .catch(function() {
                clearTimeout(timer); if (failed) return;
    ...
    window["plugin_adultjs_" + e + "_ready"] || function() {
      function t() {
        var t = $('<li class="menu__item selector" data-action="adultjs"><div class="menu__ico"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="512" height="512" x="0" y="0" viewBox="0 0 29.461 29.461" style="enable-background:new 0 0 512 512" xml:space="preserve"><g><path d="M28.855 13.134c-.479 0-.91-.197-1.371-.452-1.671 7.509-10.383 11.899-12.765 12.972-2.514-1.125-12.034-5.916-12.963-14.188-.043.029-.088.056-.132.084-.411.269-.797.523-1.299.523-.064 0-.121-.029-.184-.038C1.586 22.377 14.72 27.47 14.72 27.47s12.227-4.74 14.386-14.362a1.397 1.397 0 0 1-.251.026z" fill="currentColor"></path><path d="M29.379 8.931C28.515-.733 16.628.933 14.721 6.432 12.814.932.928-.733.062 8.931c-.397 4.426 1.173.063 3.508 1.205 1.008.494 1.99 2.702 3.356 2.974 1.998.397 3.109-1.551 4.27-1.631 3.174-.222 2.394 6.596 5.424 5.586 1.961-.653 2.479-3.016 4.171-2.806 1.582.195 3.296-3.711 4.78-3.571 2.471.23 4.305 3.786 3.808-1.757z" fill="currentColor"></path><path d="M14.894 21.534c2.286 0-.929-3.226-.588-4.511-1.994 1.276-1.697 4.511.588 4.511z" fill="currentColor"></path></g></svg></div><div class="menu__text">' + Lampa.Lang.translate("lampac_adultName") + "</div></li>"),
            a = $("<div>JS</div>");
        ...
        // ── Кнопка фильтра ───────────────────────────────────
        (function() {
          var tt, aa, nn = $('<div class="head__action head__settings selector"><svg height="36" viewBox="0 0 38 36" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="1.5" y="1.5" width="35" height="33" rx="1.5" stroke="currentColor" stroke-width="3"></rect><rect x="7" y="8" width="24" height="3" rx="1.5" fill="currentColor"></rect><rect x="7" y="16" width="24" height="3" rx="1.5" fill="currentColor"></rect><rect x="7" y="25" width="24" height="3" rx="1.5" fill="currentColor"></rect><circle cx="13.5" cy="17.5" r="3.5" fill="currentColor"></circle><circle cx="23.5" cy="26.5" r="3.5" fill="currentColor"></circle><circle cx="21.5" cy="9.5" r="3.5" fill="currentColor"></circle></svg></div>');
          nn.hide().on("hover:enter", function() {
            tt && (Lampa.Manifest.app_digital >= 300 ? tt.activity.component.filter() : tt.activity.component().filter())
          });
          $(".head .open--search").after(nn);
          Lampa.Listener.follow("activity", function(r) {
            "start" == r.type && (tt = r.object),
              clearTimeout(aa),
              aa = setTimeout(function(){ tt && tt.component !== "sisi_view_" + e && (nn.hide(), tt = !1) }, 1e3),
              "start" == r.type && r.component == "sisi_view_" + e && (nn.show(), tt = r.object)
          })
        })();
    ...
    return _createClass(e, null, [{
      ...
      value: (t = _asyncToGenerator(_regenerator().m(function t(a, n, r) {
        ...
        return _regenerator().w(function(t) {
          for(;;) switch(t.n) {
            case 0: if(!e.isAndroid){ t.n=1; break } return t.a(2, e.Native(a));
            case 1: return i=e.ensureHeaders(n), o={method:"GET",headers:i}, t.n=2, requestWithRetry(a,o,2);
            case 2: s=t.v; if(r==null){ t.n=4; break } return t.n=3, s.arrayBuffer();
            case 3: return t.a(2, new TextDecoder(r).decode(t.v));
            case 4: return t.n=5, s.text();
            case 5: return t.a(2, t.v)
          }
        }, t)
    ...
    return _createClass(e, [{
      ...
      value: (t = _asyncToGenerator(_regenerator().m(function e(t) {
        ...
        return _regenerator().w(function(e) {
          for(;;) switch(e.n) {
            case 0: return e.n=1, l.Get(t.replace("?pg=1","").replace("pg=","page="));
            case 1: return a=e.v, e.a(2, {menu:this.Menu(t), list:this.Playlist(a)})
          }
        }, e, this)
  ...
  //   FIX: StreamLinks regex — убраны ошибочные $ в середине
  // ============================================================
  g = (a = function() {
    function e(){ _classCallCheck(this,e) }
    return _createClass(e, [{
      ...
      value: (t = _asyncToGenerator(_regenerator().m(function t(a) {
        ...
        return _regenerator().w(function(t) {
          for(;;) switch(t.n) {
            case 0: if(!a.includes("/video")){ t.n=2; break } return t.n=1, l.Get(a);
            case 1: return n=t.v, t.a(2, new h(this.StreamLinks(n), a.includes("&related")));
            case 2:
              return r=new URL(a, e.host), i=r.searchParams.get("search")||"",
                ...
                uv=this.buildUrl(e.host,i,o,s,cv), t.n=3, l.Get(uv);
            case 3: return pv=t.v, t.a(2, {menu:this.Menu(o,s), list:this.Playlist(pv)});
            case 4: return t.a(2)
          }
        }, t, this)
      })), function(e){ return t.apply(this,arguments) })
      ...
      key: "getLastMonth",
      value: function(){ var e=new Date; e.setMonth(e.getMonth()-1); return e.toISOString().slice(0,7) }
    },{
      key: "Playlist",
      value: function(t) {
        if (!t) return [];
      ...
      value: function(t, a) {
        ...
        var s=[
          ...
          new p("Гэнгбэнг",r+"?sort=".concat(t,"&c=Gangbang-69")), new p("Зрелые",r+"?sort=".concat(t,"&c=Mature-38")),
          new p("Лесбиянки",r+"?sort=".concat(t,"&c=Lesbian-26")), new p("Мамочки. МИЛФ",r+"?sort=".concat(t,"&c=Milf-19")),
          new p("Минет",r+"?sort=".concat(t,"&c=Blowjob-15")), new p("Попки",r+"?sort=".concat(t,"&c=Ass-14")),
          new p("Тинейджеры",r+"?sort=".concat(t,"&c=Teen-13")), new p("ASMR",r+"?sort=".concat(t,"&c=ASMR-229"))
        ];
      ...
      key: "StreamLinks",
      value: function(t) {
        // ── ИСПРАВЛЕНО: правильный regex для HLS ─────────────
  ...
  //   FIX: StreamLinks regex — убраны ошибочные $
  // ============================================================
  y = (n = function() {
    function e(){ _classCallCheck(this,e) }
    return _createClass(e, [{
          ...
          for(;;) switch(t.n) {
            ...
            case 2:
              return r=new URL(a,e.host), i=r.searchParams.get("search")||"",
                ...
                t.n=3, l.Get(s);
            case 3: return cv=t.v, t.a(2, {menu:this.Menu(), list:this.Playlist(cv)});
            case 4: return t.a(2)
          }
        }, t, this)
      })), function(e){ return t.apply(this,arguments) })
  ...
  //   FIX: StreamLinks regex — убраны лишние пробелы
  // ============================================================
  v = (r = function() {
    function e(){ _classCallCheck(this,e) }
    return _createClass(e, [{
      ...
      value: (t = _asyncToGenerator(_regenerator().m(function t(a) {
        ...
        return _regenerator().w(function(t) {
          for(;;) switch(t.n) {
            case 0: if(!/\/video\//.test(a)){ t.n=2; break } return t.n=1, l.Get(a);
            case 1: return n=t.v, t.a(2, new h(this.StreamLinks(n), a.includes("&related")));
            case 2:
              return r=new URL(a,e.host), i=r.searchParams.get("search")||"",
                o=r.searchParams.get("sort")||"", s=parseInt(r.searchParams.get("pg")||"1",10),
                cv=this.buildUrl(e.host,i,o,s), t.n=3, l.Get(cv);
            case 3: return uv=t.v, t.a(2, {menu:this.Menu(o), list:this.Playlist(uv)});
            case 4: return t.a(2)
          }
        }, t, this)
      })), function(e){ return t.apply(this,arguments) })
      ...
      key: "StreamLinks",
      value: function(e) {
        for(var t, a={}, n=/'([0-9]+)(p|k)':\s*'(https?:\/\/[^']+)'/g; null!==(t=n.exec(e));) {
  ...
  b = (i = function() {
    ...
    return _createClass(e, [{
      ...
      value: (a = _asyncToGenerator(_regenerator().m(function t(a) {
        ...
        return _regenerator().w(function(t) {
          for(;;) switch(t.n) {
            ...
            case 3: return s=t.v, t.a(2, {menu:this.Menu(r), list:this.Playlist(s)});
            case 4: return t.a(2)
          }
        }, t, this)
      ...
      value: (t = _asyncToGenerator(_regenerator().m(function t(a) {
        ...
        return _regenerator().w(function(t) {
          for(;;) switch(t.n) {
            case 0: if(a){ t.n=1; break } return t.a(2, new m({},[]));
            case 1: return t.n=2, l.Get("".concat(e.host,"/").concat(a,"/"));
            case 2:
              if(n=t.v, r=c.extract(n,/(https?:\/\/[^ ]+\/playlist\.m3u8)/)){ t.n=3; break }
              return t.a(2, new m({},[]))
            case 3: return t.a(2, new m({auto:r.replace(/\\u002D/g,"-").replace(/\\/g,"")}, []))
          }
        }, t)
      })), function(e){ return t.apply(this,arguments) })
    }]);
  ...
  f = (o = function() {
    ...
    return _createClass(e, [{
      ...
      value: (a = _asyncToGenerator(_regenerator().m(function t(a) {
        ...
        return _regenerator().w(function(t) {
          for(;;) switch(t.n) {
            case 0: if(!a.includes("/video")){ t.n=2; break } return pv=h, t.n=1, this.StreamLinks(e.host,a);
            case 1: return dv=t.v, mv=a.includes("&related"), t.a(2, new pv(dv,mv));
            case 2:
              return n=new URL(a,e.host), r=n.searchParams.get("search")||"",
                ...
                t.n=3, l.Get(c2);
            case 3: return u2=t.v, t.a(2, {menu:this.Menu(r,i,o2), list:this.Playlist(u2)});
            case 4: return t.a(2)
          }
        }, t, this)
      })), function(e){ return a.apply(this,arguments) })
      ...
      value: (t = _asyncToGenerator(_regenerator().m(function e(t,a) {
        ...
        return _regenerator().w(function(e) {
          for(;;) switch(e.n) {
            case 0: if(a){ e.n=1; break } return e.a(2, new m({},[]))
            case 1: return e.n=2, l.Get(a);
            case 2: if(n=e.v){ e.n=3; break } return e.a(2, new m({},[]))
            case 3:
              if(r=c.extract(n,/vid ?= ?'([^']+)'/), i=c.extract(n,/hash ?= ?'([^']+)'/), r&&i){ e.n=4; break }
              return e.a(2, new m({},[]))
            case 4:
              return o2="".concat(t,"/xhr/video/").concat(r,"?hash=").concat(this.convertHash(i),"&domain=")
                .concat(t.replace(/^https?:\/\//,""),"&fallback=false&embed=false&supportedFormats=dash,mp4&_=")
                .concat(Math.floor(Date.now()/1e3)),
                e.n=5, l.Get(o2);
            case 5: if(s=e.v){ e.n=6; break } return e.a(2, new m({},[]))
            case 6:
        ...
        return _regenerator().w(function(e) {
          for(;;) switch(e.n) {
            ...
            case 2:
              ...
              try{ h2=new Function("html",a.view.eval), (g2=h2(t))&&(s2.auto=g2.replace(/&amp;/g,"&").replace(/\\/g,"")) }
            ...
            case 10: if(!M2){ e.n=11; break } return e.a(3,12);
            case 11: e.n=7; break;
        ...
        return _regenerator().w(function(e) {
          for(;;) switch(e.n) {
            ...
            case 1:
              ...
              if("view"!==(i=a.searchParams.get("mode")||"list")&&"related"!==i){ e.n=5; break }
            ...
            case 4: return p2=e.v, e.a(2, new h(p2,"related"===i||s2.includes("&related")));
            case 5:
  ...
  // ============================================================
  // [BLOCK:14] ROUTING + MirrorResolver init
  // ============================================================
  ...
  !function() {
    function e() {
      return (e = _asyncToGenerator(_regenerator().m(function e(t) {
        ...
        return _regenerator().w(function(e) {
          for(;;) switch(e.n) {
            case 0:  if(!t.startsWith(d.host)){ e.n=2; break }  return e.n=1,  z.Invoke(t);
            case 1:  case 3: case 5: case 7: case 9: case 11: case 13: case 15: return e.a(2, e.v);
            case 2:  if(!t.startsWith(g.host)){ e.n=4; break }  return e.n=3,  L.Invoke(t);
            case 4:  if(!t.startsWith(y.host)){ e.n=6; break }  return e.n=5,  j.Invoke(t);
            case 6:  if(!t.startsWith(v.host)){ e.n=8; break }  return e.n=7,  M.Invoke(t);
            case 8:  if(!t.startsWith(b.host)){ e.n=10; break } return e.n=9,  T.Invoke(t);
            case 10: if(!t.startsWith(f.host)){ e.n=12; break } return e.n=11, A.Invoke(t);
            case 12: if(!t.startsWith("nexthub://")){ e.n=14; break } return e.n=13, I.Invoke(t);
            case 14:
            ...
            case 16: return e.a(2, "unknown site")
          }
        }, e)
      }))).apply(this, arguments)
    }

    // ── [v2.0.0] Применяем зеркала + пересоздаём инстансы ───
    (function() {
      MirrorResolver.resolve().then(function(mirrors) {
        ...
        console.log('[AdultJS v2.0.0] Mirrors applied:', mirrors);
      }).catch(function(err) {
        console.warn('[AdultJS v2.0.0] MirrorResolver error:', err);
      });
    })();

    // ── [v1.3.1] Статусы источников ─────────────────────────
    window.AdultJS_Status = window.AdultJS_Status || (function() {
      var SK = 'adultjs_site_status';
      var _dot = { green: '🟢', yellow: '🟡', red: '🔴' };
      function _load() { try { var r = Lampa.Storage.get(SK,'{}'); return typeof r==='string'?JSON.parse(r):(r||{}); } catch(e){ return {}; } }
      function _save(s) { try { Lampa.Storage.set(SK, JSON.stringify(s)); } catch(e){} }
      return {
        set: function(name,status){ var s=_load(); s[name.toLowerCase()]=status; _save(s); },
        dot: function(name){ var s=_load(); var v=s[name.toLowerCase()]; return _dot[v]||_dot.green; },
        get: function(name){ var s=_load(); return s[name.toLowerCase()]||'green'; },
    ...
    window.AdultJS = {
      Menu: function() {
        ...
        P.filter(function(e){ return e.enable; }).forEach(function(t) {
          ...
          items.push({ title: window.AdultJS_Status.dot(name)+' '+name, playlist_url: 'nexthub://'+t.displayname+'?mode=list' });
        });
        items.push({ title: '📝 Диагностика источников', playlist_url: '__debug__',   debug_action:  true });
        items.push({ title: '🔄 Обновить зеркала',       playlist_url: '__mirrors__', mirror_action: true });
        return items;
      },
  ...
  window.AdultJS_Debugger = (function() {
    ...
    function getAllSources() {
      ...
      return fixed.concat(P.filter(function(c){ return c.enable; }).map(function(c){ return { name:c.displayname, url:c.host }; }));
    }
    ...
    function notifyQueue(messages) {
      ...
      function showNext() {
        ...
        i++; setTimeout(showNext, isLast?NOTIFY_LAST_MS:NOTIFY_PAUSE_MS);
      }
      showNext();
    }

    function checkSource(sourceObj) {
      var name=sourceObj.name, url=sourceObj.url, testUrl=url;
      ...
      return new Promise(function(resolve) {
        ...
        requestWithRetry(testUrl,{ method:'GET', headers:{'User-Agent':'Mozilla/5.0'} },1)
          .then(function(resp) {
            clearTimeout(timer);
            ...
            return resp.text().then(function(html) {
              ...
              try {
                ...
                } else { var mm=html.match(/data-src=/g); cardCount=mm?mm.length:0; }
    ...
    function _applyStatuses(results) {
      var SK='adultjs_site_status', store={};
      try{ var raw=Lampa.Storage.get(SK,'{}'); store=typeof raw==='string'?JSON.parse(raw):(raw||{}); } catch(e){ store={}; }
      results.forEach(function(r){ store[r.name.toLowerCase()]=!r.ok?'red':(r.cards>=0&&r.cards<3)?'yellow':'green'; });
      try{ Lampa.Storage.set(SK, JSON.stringify(store)); } catch(e){}
    }

    function runAll() {
      var sources=getAllSources(), total=sources.length, idx=0, results=[];
      ...
      function checkNext() {
        if(idx>=total) {
          var ok=results.filter(function(r){return r.ok;}),
            ...
            warned=results.filter(function(r){return r.ok&&r.cards>=0&&r.cards<3;});
          ...
          var dLines=[], hLines=[];
          failed.forEach(function(r){ dLines.push('🔴 '+r.name+': '+r.error+' [fails:'+SourceHealth.getFailCount(r.name)+']'); });
          warned.forEach(function(r){ dLines.push('🟡 '+r.name+': карточек='+r.cards); });
          var allH=SourceHealth.getAll();
          Object.keys(allH).forEach(function(n){ var rate=Math.round(SourceHealth.getSuccessRate(n)*100); if(rate<100) hLines.push(n+': '+rate+'%'); });
          var queue=[{ text:summary }];
          if(dLines.length>0) queue.push({ text:dLines.join('\n') });
          if(hLines.length>0) queue.push({ text:'📊 SuccessRate:\n'+hLines.join('\n') });
          queue.push({ text:'🟢 ОК  🟡 мало карточек  🔴 недоступен\nЗначки обновлены в меню «Сайты»' });
          notifyQueue(queue); return;
        }
        checkSource(sources[idx++]).then(function(r){ results.push(r); checkNext(); });
      }
      checkNext();
    }
    ...
    function runMirrorCheck() {
      ...
      MirrorResolver.resolve(true).then(function(mirrors) {
        var lines=Object.keys(mirrors).map(function(k){ return k+': '+(mirrors[k]||'❌'); });
        notifyQueue([{ text:'✅ Зеркала обновлены ('+lines.length+' сайтов)' },{ text:lines.join('\n') }]);
      }).catch(function(err){ notify('❌ Ошибка: '+(err.message||err)); });
    }
